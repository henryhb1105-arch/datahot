...
test("button reveals player, plays, pauses and updates the selected rate", async () => {
  ...
  assert.equal(openLabel.textContent, "暂停");
  assert.equal(open.attributes["aria-label"], "暂停");
  toggle.listeners.click();
  assert.equal(audio.paused, true);
  assert.equal(toggle.textContent, "继续");
  assert.equal(openLabel.textContent, "继续");
  assert.equal(open.attributes["aria-label"], "继续");
  rate.value = "1.5";
  ...
  assert.equal(openLabel.textContent, "速听");
  assert.equal(open.attributes["aria-label"], "速听");
});
