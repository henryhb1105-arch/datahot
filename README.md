...
[`cases.html`](https://datahot.xiahongbin.com/cases.html) 不是另一条新闻流，而是解决真实设计问题的工作参考库。页面先按入口与提问、任务编排、结果表达、可信与溯源、语义与上下文、治理与评估六类问题进入，再用产品形态、用户任务和关键词缩小范围；每张卡片直接给出一个可借鉴结论，并可选择 2–3 个案例横向比较。详情页在保留原始图文与原文链接的基础上，补充用户问题、功能模块、关键交互、收益与代价、可借鉴点和限制。
